...
Random-effects meta-analysis across 19 studies revealed a phase advance trend under spaceflight (pooled shift = −0.08 h, 95% CI [−0.17, +0.01], p = 0.092, I² = 86.2%). Heterogeneity was high, reflecting the diversity of study designs, tissues, hardware, and light regimes. The pooled effect corresponds to approximately a 4.6-minute phase advance, which, while small in absolute terms and nominally non-significant at the conventional threshold after inclusion of OSD-522, is consistent in direction across the majority of studies.

### Stratified analysis

Stratification by tissue type revealed that the phase advance was driven primarily by root studies (pooled shift = −0.17 h, 95% CI [−0.24, −0.10], p < 0.001, I² = 0.0%, k = 4: OSD-120, OSD-193, OSD-218, OSD-281), which showed remarkably low heterogeneity (Figure 5). Whole seedling studies showed a larger but non-significant trend (−0.19 h, p = 0.106, I² = 91.1%), while leaf studies showed a non-significant phase delay (+0.18 h, p = 0.100). Shoot studies (k = 3: OSD-121, OSD-437, OSD-522) showed a pooled phase shift of +0.13 h (p = 0.413, I² = 79.2%). Hypocotyl studies showed no significant effects.

Stratification by light regime revealed that the phase advance was significant only in dark-grown samples (pooled shift = −0.20 h, 95% CI [−0.38, −0.01], p = 0.036, I² = 93.2%, k = 6: OSD-37, OSD-38, OSD-44, OSD-121, OSD-205, OSD-321) and in samples with unknown light regimes (−0.09 h, p = 0.037, I² = 58.6%, k = 7). Continuous light samples showed no significant shift (+0.002 h, p = 0.978), and LD 16:8 samples showed a non-significant trend toward delay (+0.14 h, p = 0.083). OSD-522 was grown under a unique LD 4:2 light regime (k = 1).
...
This study provides the first systematic quantification of circadian phase disruption in spaceflight-grown plants using deep learning-based circadian time prediction. Our meta-analysis of 19 Arabidopsis spaceflight transcriptomics studies, including the newly integrated BRIC-LED experiment (OSD-522), reveals a consistent directional circadian phase advance under spaceflight conditions (pooled shift ≈ −0.08 h, approximately 4.6 minutes, p = 0.092). While the overall pooled effect is nominally non-significant after inclusion of OSD-522, the curated Fork A analysis retains significance (−0.09 h, p = 0.008), and the tissue-stratified root analysis remains highly significant across both forks.

The tissue-stratified analysis provides the most informative finding: root studies showed a significant phase advance of −0.17 h (p < 0.001, I² = 0.0%, k = 4: OSD-120, OSD-193, OSD-218, OSD-281) with remarkably low heterogeneity, suggesting a tissue-specific and highly reproducible circadian response to spaceflight. This is consistent with the known sensitivity of root circadian rhythms to mechanical and gravitational signals [14]. Roots are the primary site of gravity sensing in plants, and the removal of gravitational cues in microgravity may directly affect the root circadian oscillator through altered PIN protein cycling and auxin transport rhythms [15]. In contrast, shoot tissues (OSD-121, OSD-437, OSD-522) exhibited a modest, non-significant phase delay (+0.13 h, p = 0.413), with OSD-522 (grown in BRIC-LED hardware under an LD 4:2 photoperiod) showing a +0.66 h shift (p = 0.064).
...
The high heterogeneity observed in the overall meta-analysis (I² = 86.2%) reflects the substantial diversity in study designs, including different hardware systems, ecotypes, tissues, light regimes, and growth durations. This heterogeneity is both a limitation and a strength: it demonstrates that the circadian phase advance is not driven by a single experimental condition, but it also means that the pooled effect size should be interpreted as an average across diverse conditions rather than a precise estimate applicable to any specific scenario. The contrasting heterogeneity between the root stratum (I² = 0%) and whole-seedling stratum (I² = 91%) underscores that tissue type is a key moderator of the spaceflight circadian response.
...
[6] Nayak, K.K., et al. (2020) Spaceflight-induced changes in the Arabidopsis circadian clock. *npj Microgravity* 6, 21.
...
[8] Barker, R., et al. (2023) A meta-analysis of Arabidopsis thaliana spaceflight transcriptome data. *npj Microgravity* 9, 23.
...
[11] Reynolds, W., et al. (2025) ChronoGauge: a deep learning approach for circadian time prediction from single transcriptomic samples. *bioRxiv* 2025.01.12.573912.
