...
\begin{table}[htbp]
\centering
\caption{\textbf{Dataset characteristics.} Summary of all studies included in the analysis.}
\small
...
\begin{table}[htbp]
\centering
\caption{\textbf{Per-study circadian phase shift results.} Watson-Williams test for each study with paired flight and ground controls.}
\small
...
\begin{table}[htbp]
\centering
\caption{\textbf{Meta-analysis results by stratum.} Random-effects meta-analysis (REML) of phase shifts across studies.}
\small
...
\caption{\textbf{PCA of ChronoGauge sub-model predictions.} (\textbf{A}) PCA colored by condition (flight vs ground control). (\textbf{B}) PCA colored by study.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/figS2_clock_genes.png}
\caption{\textbf{Core clock gene expression heatmap.} Z-scored expression of core circadian clock genes (CCA1, LHY, TOC1, PRR9/7/5, GI, ELF4, ELF3, LUX, TIC) across all samples, sorted by condition and study.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/figS3_phase_by_hardware.png}
\caption{\textbf{Phase shift by hardware type.} Box plots showing the distribution of per-study phase shifts grouped by hardware configuration.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/figS4_phase_by_light_regime.png}
\caption{\textbf{Phase shift by light regime.} Box plots showing the distribution of per-study phase shifts grouped by light regime.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/figS5_fork_comparison.png}
\caption{\textbf{Fork A versus Fork B comparison.} Per-study phase shifts for Fork A (Barker 2023 cohort, 14 studies) and Fork B (all Arabidopsis spaceflight transcriptomics, 24 studies).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/figS6_model_uncertainty.png}
\caption{\textbf{Model uncertainty analysis.} (\textbf{A}) Relationship between phase shift magnitude and circular variance. (\textbf{B}) Number of ensemble sub-models used per study.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/figS7_circadian_trajectory.png}
\caption{\textbf{Circadian trajectory analysis.} (\textbf{A}) t-SNE embedding of 100-dimensional ChronoGauge sub-model predictions, colored by condition (flight vs ground control). (\textbf{B}) t-SNE embedding colored by study. (\textbf{C}) PCA of sub-model predictions colored by condition. (\textbf{D}) t-SNE embedding colored by predicted circadian time.}
\end{figure}

\renewcommand{\thefigure}{S7b}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figures/figS7b_centroid_vs_shift.png}
\caption{\textbf{Centroid distance versus phase shift.} t-SNE centroid distance (Euclidean distance between flight and ground centroids) plotted against absolute phase shift magnitude for 18 studies. Spearman $\rho = 0.633$, $p = 0.005$.}
...
\caption{\textbf{Gene set enrichment of phase-shifted studies.} (\textbf{A}) Normalized enrichment score (NES) for the circadian rhythm pathway (GO:0007623) across the four significant phase-shift studies. OSD-193 shows significant downregulation (NES = $-$2.25, FDR = 0.004). (\textbf{B}) Top enriched pathways (FDR $< 0.05$) for OSD-321, the study with the largest phase shift. (\textbf{C}) GSEA running enrichment score for the circadian rhythm pathway in OSD-193.}
\end{figure}
